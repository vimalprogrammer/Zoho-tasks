...
//out.println("<td><input type=\"submit\" name=\"seats\" value="+seats+" /></td>");
     ...
     out.println("<tr><td><input type=\"submit\" name=\"bus_no\" value="+bus_no+"><td>" + bus_name + "</td><td>" + seats + "</td><td>" + sleeper + "</td><td>" + dep + "</td><td>" + des+"</td></tr>");
 } while(rs.next());
...
{
    String message = "No bus available with this spots";
    request.setAttribute("message", message);
    request.getRequestDispatcher("book_ticket1.jsp").forward(request, response);    
    stmt.close();
    c.close();
